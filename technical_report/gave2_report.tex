...
\begin{tikzpicture}[
  ...
  \node[box, below=0.5cm of rrwnet, fill=blue!20] (refine1) {Recursive\\refinement $\times(5{+}1)$};
  ...
  \node[box, below=0.5cm of cmrrwnet, fill=orange!20] (refine2) {Recursive\\refinement $\times(5{+}1)$};
...
5-fold cross-validation, plain random splitting at the case level (each of
the 50 training cases -- CFP plus its paired FFA images and labels --
assigned wholly to one fold, so no case appears in both a fold's training
and validation split). No camera-vendor stratification, since no vendor
metadata ships with the dataset. To build a
...
from seed 202, plus horizontal-flip TTA within each checkpoint. The final
Task 1 ensemble uses exactly these 11 checkpoints; the final Task 2
ensemble uses these same 11 plus one additional checkpoint (12 total,
described below). We found empirically
...
\textbf{Model size and wall-clock cost.} Task 1 (RRWNet) has 62.06M unique
parameters (237\,MB fp32 checkpoint); Task 2 (CMRRWNet) has 82.39M unique
parameters (315\,MB fp32 checkpoint), the extra $\sim$20M coming from the
two additional FFA encoder branches. Training throughput on the primary
T550 GPU is
$\sim$8--10\,s/gradient step (measured directly from \texttt{train\_log.csv}
across multiple folds of both tasks), i.e.\ $\sim$7--8\,min/epoch at 50
steps/epoch, so a single 60-epoch fold takes roughly 7--8\,h ($\sim$8.75\,h
for the first, 70-epoch Task 1 fold). Full-resolution tiled sliding-window
inference on this GPU (single checkpoint, single image, no TTA) measured
61.6\,s for Task 1 and 60.9\,s for Task 2 -- essentially the same despite
Task 2's larger model, since its smaller 320\,px tiling roughly offsets the
extra two-encoder-branch cost. With the flip-TTA pass included (measured
directly for Task 1 at 63.4\,s for the mirrored pass, assumed comparable
for Task 2 given the matching no-TTA numbers), one checkpoint takes
$\sim$125\,s/image for either task. Since ensemble inference is sequential
per checkpoint, this scales close to linearly: the full 11-member Task 1
ensemble with TTA is $\sim$23\,min/image, and the 12-member Task 2 ensemble
is $\sim$25\,min/image.
...
Note on Task1/Task2 wobble across rows not targeting that task (e.g.\
$7.261\!\to\!7.256$ between rows 2--3, $7.285\!\to\!7.291$ between rows
4--5): these prediction files are byte-identical between the two
submissions in each pair (sha256-verified) -- the pixel-level metrics
(Sen/Spec/Acc/DSC) match exactly, but the composite Task1/Task2 scores still
shift by up to $\pm0.03$. This is organizer-side sampling noise in the
COR/INF topology metrics, not an effect of the row's labeled change; see
\texttt{submissions\_log.csv} for the specific per-submission comparisons.
Separately, row 1's values are exactly as shown on the official leaderboard
(verified against the raw per-submission export); unlike every later row,
they do not satisfy the 0.2/0.4/0.4 weighting to full precision (gap
$\approx$0.022, unexplained, not a transcription error).
