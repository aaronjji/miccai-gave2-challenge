...
multi-seed cross-validation ensemble (11 members across three independent
random partitions), horizontal-flip test-time augmentation, and a
...
\keywords{Retinal vessel segmentation \and Artery/vein classification \and
Topology-preserving loss \and Ensemble learning \and Vascular biomarkers.}
\end{abstract}
%
%
%
...
\begin{tikzpicture}[
  ...
  io/.style={draw, rounded corners, minimum height=0.7cm, minimum width=2.2cm, align=center, font=\scriptsize, fill=gray!10},
  arr/.style={-Stealth, thick}
]
  % Task 1
  \node[io] (cfp1) {CFP image\\(3ch, 1536$\times$1024)};
  \node[box, right=0.6cm of cfp1, fill=blue!8] (rrwnet) {RRWNet\\encoder-decoder};
  \node[box, right=0.6cm of rrwnet, fill=blue!15] (refine1) {Recursive\\refinement $\times 5$};
  \node[io, right=0.6cm of refine1] (out1) {A/V/vessel\\probability map};
  ...
  % Task 2 (below)
  \node[io, below=1.3cm of cfp1] (cfp2) {CFP image\\(3ch)};
  \node[io, below=0.25cm of cfp2] (ffa) {FFA-A / FFA-AV\\(registered, 2ch)};
  \node[box, right=0.6cm of cfp2, yshift=-0.55cm, fill=orange!10] (cmrrwnet) {CMRRWNet:\\3-branch encoder\\+ SE-gated fusion};
  \node[box, right=0.6cm of cmrrwnet, fill=orange!20] (refine2) {Recursive\\refinement $\times 5$};
  \node[io, right=0.6cm of refine2] (out2) {A/V/vessel\\probability map};

  \draw[arr] (cfp2) -- (cmrrwnet);
  \draw[arr] (ffa) -- (cmrrwnet);
  \draw[arr] (cmrrwnet) -- (refine2);
  \draw[arr] (refine2) -- (out2);

  % Ensemble
  \node[box, below right=1.6cm and -0.3cm of out1, fill=green!10, minimum width=3.4cm] (ens) {Multi-seed, multi-fold\\ensemble average + TTA};
  \draw[arr] (out1) -- (ens);
  \draw[arr] (out2) -- (ens);

  % Task 3
  \node[io, below=0.9cm of ens] (odimg) {CFP image};
  \node[box, left=0.6cm of odimg, fill=purple!10] (segformer) {SegFormer\\OD detector};
  \node[box, below=0.9cm of ens, xshift=2.6cm, fill=purple!20] (biomarker) {Biomarker\\computation};
  \node[io, right=0.6cm of biomarker] (out3) {AVR, densities,\\fractal dims.};

  \draw[arr] (odimg) -- (segformer);
  \draw[arr] (segformer) |- (biomarker);
  \draw[arr] (ens) -- (biomarker);
...
across the recursive-refinement iterations:
\begin{equation}
\mathcal{L}_{\text{RR}} = \mathcal{L}_1 + \frac{1}{Z}\sum_{i=2}^{N} i \cdot \mathcal{L}_i,
\qquad Z = \tfrac{1}{2}(N{-}1)N,
\label{eq:rrloss}
\end{equation}
$\mathcal{L}_i$ being the per-iteration BCE3 loss (independent weighted BCE
...
(\texttt{pamixsun/segformer\_for\_optic\_disc\_cup\_segmentation}, used
...
and 50 unlabeled cases used for leaderboard evaluation.
...
probabilities (quantized to 32 levels before saving) across 11 checkpoints:
5 folds $\times$ seed 77, 5 folds $\times$ seed 101, and 1 fold from seed
202, plus horizontal-flip TTA within each checkpoint. We found empirically
...
this). $\texttt{pos\_weight}=5.0$, $\texttt{vein\_pos\_weight}=6.0$,
$\texttt{cldice\_weight}=0.3$, $\texttt{vein\_topology\_ratio}=1.3$ (see
Sect.~1.2 for the loss definition). Checkpoints saved every 5 epochs;
validation (on the fold's held-out split) every 5 epochs.
